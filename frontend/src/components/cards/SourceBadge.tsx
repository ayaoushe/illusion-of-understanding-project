interface SourceBadgeProps {
  type: 'guideline' | 'pubmed' | 'doi' | 'rct' | 'review';
  disabled?: boolean;
}

export function SourceBadge({ type, disabled = false }: SourceBadgeProps) {
  const labels = {
    guideline: 'Guideline',
    pubmed: 'PubMed',
    doi: 'DOI',
  ...
  return (
    <span
      className={`source-badge source-${type} ${disabled ? 'disabled' : ''}`}
      style={{
