...
export function buildPatientView(c) {
    ...
    const metastases = METASTASIS_SITES.map(([key, label]) => ({
        site: label,
        present: yesNo(f[key]),
    }));
    ...
    const qolConcerns = ['Maintaining daily functioning'];
    qolConcerns.push(ageForLogic >= 65 ? 'Preserving independence at home' : 'Continuing to work during treatment');
    if (her2 || !hr)
        qolConcerns.push('Concern about hair loss and neuropathy from chemotherapy');
    if (hr)
        qolConcerns.push('Menopausal symptoms from endocrine therapy');
    if (isMetastatic)
        qolConcerns.push('Managing fatigue and overall symptom burden');
    if (isSmoker)
        qolConcerns.push('Support to stop smoking');
    ...
    const missingData = [];
    if (her2)
        missingData.push('LVEF (baseline cardiac function) not yet documented');
    missingData.push('Ki-67 proliferation index pending');
    if (hr && !isMetastatic)
        missingData.push('Genomic recurrence score (Oncotype/MammaPrint) outstanding');
    if (isMetastatic)
        missingData.push('Biopsy re-confirmation of receptor status at metastatic site pending');
    if (nodePos)
        missingData.push('Axillary staging (sentinel vs. dissection) to be finalized');
    if (isSmoker)
        missingData.push('Pulmonary function assessment pending');
    return {
